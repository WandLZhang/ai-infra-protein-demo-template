...
import InfraMap, { type ZoneInfo, HOME_POSITION } from './components/InfraMap'
...
import { config, theme, accentAlpha } from './config'
...
type Phase = 'home' | 'dispatching' | 'running' | 'catalog' | 'catalog2' | 'catalog3' | 'catalog4' | 'md1' | 'md2' | 'md3' | 'pd1' | 'pd2' | 'img' | 'tpu1' | 'tpu2' | 'tpu3' | 'models1' | 'models2' | 'models3' | 'done'
...
export default function App() {
  ...
  React.useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'Enter' || e.key === 'ArrowRight') {
        ...
        // 'models3' is the final manual slide — 'done' is set by polling when inference completes
        // 'done' stays
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', background: '#171717', overflow: 'hidden', position: 'relative' }}>
      ...
      <InfraMap lanes={lanes} zoneStates={zoneStates} vmStates={vmStates} onZoneClick={setSelectedZone} center={mapCenter} zoom={mapZoom} highlightUS={phase === 'catalog2' || phase === 'catalog3' || phase === 'catalog4'} showSpokes={phase === 'catalog2' || phase === 'catalog3' || phase === 'catalog4'} showHalos={phase === 'catalog3' || phase === 'catalog4'} mdLayer={mdLayer} showHyperdiskHub={showHyperdiskHub} showPartitionChips={showPartitionChips} showSliceViz={showSliceViz} />
      ...
      <div className="location-paper" key={phase === 'home' ? 'loc-home' : 'loc-cloud'}>
        ...
        <div className="location-paper-coords" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <SetupWizard visible={phase === 'home'} />
        </div>
      </div>
      ...
      <InfoButton
        ...
        title={
          phase === 'home' ? 'Cornell AI Initiative' :
          phase === 'dispatching' ? 'Multi-Region Burst' :
          phase === 'running' ? 'Multi-Region Burst' :
          phase === 'catalog' ? 'Cornell Research Applications Catalog' :
        ...
        sections={
          phase === 'home' ? [
            { body: 'Cornell\'s <a href="https://ai.cornell.edu/initiative/" target="_blank">AI Initiative</a> is standing up an on-prem GenAI cluster for Fall 2026 (Phase 1). Research compute today is spread across three independent estates — <a href="https://www.cac.cornell.edu/" target="_blank">CAC</a> (Red Cloud, Hopper), <a href="https://biohpc.cornell.edu/" target="_blank">BioHPC</a>, and Cornell Tech\'s G2 — plus shared <a href="https://www.empireai.edu/" target="_blank">Empire AI</a> allocations queued on GB200s. Phase 2 is extending that foundation elastically into the cloud.\n\nGoogle designs its own silicon, network transport, and datacenter hardware — attributes that make cloud bursting with Google worth a closer look.\n\nOn screen is a terminal on a Cornell BioHPC login node with a Slurm command: <code>sbatch predict.sh --model=all --target=both --protein=brca1 --requeue --partition=tpu,gpu</code>. When we press Enter, six inference jobs will dispatch across both TPU and GPU partitions to whichever cloud CONUS regions have capacity.' },
          ] :
          phase === 'dispatching' || phase === 'running' ? [
            { body: 'A <a href="https://cloud.google.com/network-connectivity/docs/interconnect/concepts/overview" target="_blank">400 Gbps Dedicated Interconnect</a> is possible to the nearest Google edge, with private IP connectivity, sub-millisecond latency, MACsec encrypted in transit. <a href="https://cloud.google.com/managed-microsoft-ad/docs/overview" target="_blank">Managed Microsoft AD</a> can bridge on-prem UID/GID into the cloud nodes so researchers log in with the same identity they use today.\n\nThe operating model is <b>one Slurm, one identity, one <code>/data/</code></b> — researchers submit <code>sbatch</code> the same way they do today; Managed AD bridges their UID/GID; Cloud Storage FUSE mounts the same paths. Cornell Tech\'s <a href="https://it.coecis.cornell.edu/researchit/g2cluster/g2-faqs/" target="_blank">G2</a> already runs this on-prem — owner partitions plus Slurm preemption let researchers burst onto idle shared nodes — and we extend that same burst surface to GCP.\n\nSlurm\'s <code>--requeue</code> flag handles Spot preemption: if a node is reclaimed, the job retries in the next available zone. Researchers see none of this — they submit <code>sbatch</code> and Slurm handles the rest.' },
          ] :
          phase === 'catalog' ? [
            { body: 'Cornell\'s research workloads — like the <a href="https://biohpc.cornell.edu/lab/labsoftware.aspx" target="_blank">1,300+ titles in the BioHPC software catalog</a> — span the CAC, BioHPC, and G2 estates. We organize them into three workload shapes, ordered from quickest cloud win to deepest HPC:\n\n<ul style="margin: 8px 0; padding-left: 18px; list-style-type: none;"><li style="margin-bottom: 10px; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>Independent GPU Jobs</b></li><li style="margin-bottom: 10px; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>Data-Anchored Burst</b></li><li style="margin-bottom: 0; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>Tightly-Coupled Simulation</b></li></ul>' },
          ] :
          phase === 'catalog2' ? [
            { body: 'Input datasets are large — hundreds of GB per session — but static per experiment, making this an ideal burst profile. The compute is often single-GPU per task; the prework is incrementally syncing predetermined parts of the shared filesystem using <a href="https://cloud.google.com/storage-transfer/docs/overview" target="_blank">Storage Transfer Service</a>.\n\n<ul style="margin: 8px 0; padding-left: 18px; list-style-type: none;"><li style="margin-bottom: 10px; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>Nozomi Ando</b> (Chemistry and Chemical Biology) — canonical CHESS user. Her BioSAXS work at the MacCHESS beamline, analyzed in BioXTAS RAW (whose EFA/REGALS deconvolution her group authored), turns each beamtime into a write-heavy burst of detector images followed by read-intensive reduction. CHESS runs 1,100 scientists/year through the synchrotron, generating hundreds of TB per run on a 400-core Grid Engine farm where detector brightness is outrunning compute, and already moves data via Globus.</li><li style="margin-bottom: 10px; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>Christopher Fromme</b> (Molecular Biology and Genetics) — multi-terabyte cryo-EM movie datasets of the Golgi trafficking machinery. Written once, then read repeatedly through CryoSPARC/RELION motion-correction and 3D classification.</li><li style="margin-bottom: 10px; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>John Lis</b> (Molecular Biology and Genetics) — invented the PRO-seq nascent-transcription method. Decades of immutable sequencing archives reprocessed in batch through BWA and dREG.</li><li style="margin-bottom: 0; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>Eun-Ah Kim</b> (Physics) — terabyte-scale quantum-matter X-ray imaging. Same write-once, read-many profile.</li></ul>\nCompute nodes in every burst region mount the same <code>/data/</code> paths from this one multi-region bucket via <a href="https://cloud.google.com/storage/docs/cloud-storage-fuse/overview" target="_blank">Cloud Storage FUSE</a> — one shared namespace everywhere, read anywhere and write results back.' },
          ] :
          phase === 'catalog3' ? [
          ...
          phase === 'md1' ? [
            { body: 'Tightly-coupled MPI simulation — molecular dynamics, computational fluid dynamics, and finite-element multiphysics. Multi-node, latency-sensitive, anchored in one region.\n\n<ul style="margin: 8px 0; padding-left: 18px; list-style-type: none;"><li style="margin-bottom: 10px; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>Brian Crane</b> (Chemistry and Chemical Biology) — replica-exchange molecular dynamics of multidomain signaling proteins like the CheA chemotaxis kinase. Many tightly-coupled ranks exchanging state, the canonical communication-bound HPC pattern.</li><li style="margin-bottom: 10px; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>Itai Cohen</b> (Physics) — coupled-physics finite-element models in COMSOL of microscale actuators and biological-tissue mechanics.</li><li style="margin-bottom: 0; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>Jian-Xun Wang</b> (Mechanical and Aerospace Engineering) — at Cornell since 2025. Builds differentiable CFD solvers in <a href="https://jax.readthedocs.io/" target="_blank">JAX</a>, Google\'s own framework, embedding PDE operators directly into neural nets as the ML-native complement to classical MPI fluid dynamics.</li></ul>\nThe hot scratch tier is zonal. Two options serve this profile: <a href="https://docs.cloud.google.com/managed-lustre/docs/overview" target="_blank">Managed Lustre</a> with full POSIX, sub-millisecond latency at <b>10 TB/s</b> (AWS FSx for Lustre caps around 2 TB/s) or <a href="https://docs.cloud.google.com/storage/docs/rapid/rapid-bucket" target="_blank">Rapid Bucket</a> with <b>15 TB/s, 20 million QPS</b> suited to streaming checkpoints. This same zonal tier is where data-anchored work that needs POSIX random I/O — RELION\'s iterative 3D refinement, CryoSPARC\'s database — stages in from the shared multi-region bucket, then writes results back.\n\nOn the hierarchical-namespace Rapid Bucket, a finished job commits with an atomic, metadata-only folder rename: <code>gcloud storage mv gs://cornell-research/Refine3D/job001.staging gs://cornell-research/Refine3D/job001</code> updates the path without copying or deleting the underlying files, where AWS can only rename one object at a time, not whole directories.' },
          ] :
          phase === 'md2' ? [
          ...
          phase === 'pd1' ? [
            { body: 'Single-node, single-GPU jobs that fan out embarrassingly parallel — each job runs independently, inputs are small (KB–MB), compute is large. Every job lands on its own node via <code>sbatch</code>, Slurm fills on-prem first, and overflow bursts to whichever GCP region has capacity. The live demo runs this shape: AlphaFold, ESMFold, and Boltz-2 each on a single GPU or TPU.\n\n<ul style="margin: 8px 0; padding-left: 18px; list-style-type: none;"><li style="margin-bottom: 10px; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>Haiyuan Yu</b> (Computational Biology) — PIONEER pipeline: AlphaFold2 + ZDOCK across a 280,000-interaction human interactome, predicting how disease mutations break protein-protein interactions. Per-interaction deep learning that fans out cleanly onto single-GPU nodes.</li><li style="margin-bottom: 10px; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>Effie Apostolou</b> (Medicine, Weill Cornell Medicine) — 3D-chromatin Hi-C and HiChIP maps of cell reprogramming and cancer progression. Each sample is its own batch job.</li><li style="margin-bottom: 10px; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>Steven Josefowicz</b> (Pathology, Weill Cornell Medicine) — ChIP-seq, CUT&amp;RUN, and ATAC-seq profiling of immune epigenetic memory. Same per-sample fan-out.</li><li style="margin-bottom: 10px; padding-left: 12px; border-left: 2px solid #2a2a2a;"><b>Ekta Khurana</b> (Englander Institute, Weill Cornell Medicine) — prostate-cancer AI (pathology slides + gene-expression + outcomes) and the PyTorch-based AutoRadAI MRI study. Both ran on Empire AI Alpha\'s shared H100s — but instead of queueing on the consortium, workloads like these can land on GCP in minutes.</li></ul>\nModel weights are served from <a href="https://docs.cloud.google.com/kubernetes-engine/docs/how-to/persistent-volumes/hyperdisk-ml" target="_blank">Hyperdisk ML</a>. <b>One volume serves 2,500 instances at 1.2 TiB/s aggregate</b>. An admin creates the volume and loads weights once; researchers\' job scripts just read a local path.\n\nOne layer up at the host, <a href="https://cloud.google.com/kubernetes-engine/docs/concepts/fast-starting-nodes" target="_blank">BoltVMs</a> are pre-initialized GPU nodes that keep the boot, driver, and container runtime warm — <b>H100 cold-start drops from 15 minutes to 2</b>.\n\nAnd at the workload layer, <a href="https://docs.cloud.google.com/kubernetes-engine/docs/how-to/checkpoint-restore" target="_blank">Pod Snapshots</a> snapshot full pod state and restore in seconds — <b>80% faster warm restart for a 70B-parameter model</b>. Standard K8s pod restart in AWS EKS or AKS reloads model weights from scratch.' },
          ] :
          phase === 'pd2' ? [
